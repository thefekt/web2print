	...
	'print_template' : {
		properties : {
			...
			'document' : {
				...
				events : {
					'insert,update,delete,updateRelation' : onDocumentChange
				}
			},
			'contents' : {
				template : 'relation',
				...
				events : {
					'insert,update,delete' : onContentsChange // ONLY UPDATE TODO CHECK ! MULTIPLE > ONLY UPDATE EVENT !! TODO TODO TODO 
				}
			},
			'preview_document' : {
				template : 'relation.file_upload',
...
function onInsertobject() {
	...
	function createDir(parent,code) {
		var rparent = db.documents.folder.SELECT("code=:CODE AND parent = :ID",{CODE:code,ID:parent.id})[0];
		if (!rparent) {
...
function onDocumentChange() {
	...
	if (!this.INSERTED) {
		...
		var nparent = db.documents.folder.SELECT({where:"path=:PATH",PATH:`/web2print/${this.uuid}/defaults`})[0];
...
// print cached documents.folder
/*vr.define({
	template : require('@visionr/model/documents.folder/default'),
	path : '/system/print_cache',
	access_read : 'administrators',
	access_update : 'administrators',
	access_update : 'ROOT',
	access_delete : 'ROOT'
});*/
